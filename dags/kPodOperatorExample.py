...
collect_data = KubernetesPodOperator(
    ...
    cmds=["bash", "-c"],
    arguments=[f"pip install pandas && python {SCRIPT_PATH}/collect_data.py"],
    env_vars=env_vars,
...
preprocess_data = KubernetesPodOperator(
    ...
    image='python:3.9-slim',
    cmds=["bash", "-c"],
    arguments=[f"pip install pandas scikit-learn numpy && python {SCRIPT_PATH}/preprocess_data.py"],
    env_vars=env_vars,
    volumes=[data_volume_config, git_sync_volume_config],
    volume_mounts=[data_volume_mount, git_sync_volume_mount],
    container_resources = resources,
...
analyze_data = KubernetesPodOperator(
    ...
    image='python:3.9-slim',
    cmds=["bash", "-c"],
    arguments=[f"pip install pandas matplotlib seaborn numpy && python {SCRIPT_PATH}/analyze_data.py"],
    env_vars=env_vars,
    volumes=[data_volume_config, git_sync_volume_config],
    volume_mounts=[data_volume_mount, git_sync_volume_mount],
    container_resources = resources,
...
store_results = KubernetesPodOperator(
    ...
    image='python:3.9-slim',
    cmds=["bash", "-c"],
    arguments=[f"pip install pandas && python {SCRIPT_PATH}/store_results.py"],
    env_vars=env_vars,
    volumes=[data_volume_config, git_sync_volume_config],
    volume_mounts=[data_volume_mount, git_sync_volume_mount],
    container_resources = resources,
